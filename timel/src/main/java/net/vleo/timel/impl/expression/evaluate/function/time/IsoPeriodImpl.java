 ...
 */
public class IsoPeriodImpl extends AbstractValueFunction<String> {
    public static final String TOKEN = "Period";

    private final ValueNode<String> valueNode;

    public IsoPeriodImpl(ValueNode<String> valueNode) {
        super(TOKEN, Types.getPeriodType(), valueNode);
    ...
    public UpscalableIterator<String> evaluate(Interval interval, ExecutorContext context) {
        return new UpscalerWrapperIterator<String>(
                getType().getUpscaler(),
                new AdapterTimeIterator<String, String>(
                        valueNode.evaluate(interval, context)
                ) {
                    @Override
                    protected Sample<String> adapt(Sample<String> sample) {
                        String value = sample.getValue();

                        try {
                            Period period = Period.parse(value);
                        } catch(IllegalArgumentException e) {
                            throw new ExecutionException("Invalid period " + value);
                        }
