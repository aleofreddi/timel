...
/**
 * Double floor implementation.
 *
 * @author Andrea Leofreddi
 */
public class FloorImpl extends AbstractValueFunction<Double> {
    public static final String TOKEN = "Floor";

    private ValueNode<Double> valueNode;

    public FloorImpl(ValueNode<Double> valueNode) {
        ...
        return new UpscalerWrapperIterator<Double>(
                ...
                new AdapterTimeIterator<Double, Double>(valueNode.evaluate(interval, context)) {
                    ...
                    protected Sample<Double> adapt(Sample<Double> sample) {
                        return sample.copyWithValue(Math.floor(sample.getValue()));
                    }
                }
        );
    }
