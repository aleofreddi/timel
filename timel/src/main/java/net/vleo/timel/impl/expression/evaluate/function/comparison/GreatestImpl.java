 ...
 */
public class GreatestImpl<T extends Comparable<T>> extends AbstractValueFunction<T> {
    public static final String TOKEN = "Greatest";

    private List<ValueNode<T>> expressions;

    public GreatestImpl(ValueType<T> type, List<ValueNode<T>> arguments) {
        super(TOKEN, type, arguments);
        ...
        return new UpscalerWrapperIterator<T>(
                ...
                new AdapterTimeIterator<Object[], T>(
                    ...
                    protected Sample<T> adapt(Sample<Object[]> sample) {
                        ...
                        return sample.copyWithValue(Collections.max(args));
                    }
                }
        );
    }
