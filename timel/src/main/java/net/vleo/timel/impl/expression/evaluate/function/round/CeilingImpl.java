...
/**
 * Double ceiling implementation.
 *
 * @author Andrea Leofreddi
 */
public class CeilingImpl extends AbstractValueFunction<Double> {
    public static final String TOKEN = "Ceiling";

    private final ValueNode<Double> valueNode;

    public CeilingImpl(ValueNode<Double> valueNode) {
        ...
        return new UpscalerWrapperIterator<Double>(
                ...
                new AdapterTimeIterator<Double, Double>(valueNode.evaluate(interval, context)) {
                    ...
                    protected Sample<Double> adapt(Sample<Double> sample) {
                        return sample.copyWithValue(Math.ceil(sample.getValue()));
                    }
                }
        );
    }
