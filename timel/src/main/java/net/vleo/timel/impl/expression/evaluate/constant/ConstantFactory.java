...
import net.vleo.timel.impl.expression.evaluate.function.eval.*;
 ...
 */
public class ConstantFactory extends FunctionFactory {
    @Override
    public TreeNode instance(String id, List<TreeNode> arguments, CompilerContext context) throws ParseException {
        if(EImpl.TOKEN.equals(id)) {
            FunctionArgumentParser parser = new FunctionArgumentParser(EImpl.TOKEN, arguments);

            if(!arguments.isEmpty())
                throw new ParseException(EImpl.TOKEN + " is a constant");

            return new EImpl();

        } else if(PiImpl.TOKEN.equals(id)) {
            if(!arguments.isEmpty())
                throw new ParseException(PiImpl.TOKEN + " is a constant");
