package net.vleo.timel.impl.target.tree;

/*-
 * #%L
 * TimEL core
 * %%
 * Copyright (C) 2015 - 2019 Andrea Leofreddi
 * %%
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Lesser Public License for more details.
 * 
 * You should have received a copy of the GNU General Lesser Public
 * License along with this program.  If not, see
 * <http://www.gnu.org/licenses/lgpl-3.0.html>.
 * #L%
 */

import lombok.EqualsAndHashCode;
import lombok.ToString;
import lombok.Value;
import net.vleo.timel.executor.ExecutorContext;
import net.vleo.timel.impl.intermediate.tree.AbstractSyntaxTree;
import net.vleo.timel.impl.downscaler.Downscaler;
import net.vleo.timel.impl.upscaler.Upscaler;
import net.vleo.timel.iterator.UpscalableIterator;
import net.vleo.timel.time.Interval;
import net.vleo.timel.function.Function;

import java.util.List;

/**
 * Function call node.
 *
 * @author Andrea Leofreddi
 */
@Value
@EqualsAndHashCode(callSuper = true)
public class FunctionCall extends AbstractTargetTree {
    private final Upscaler<Object> upscaler;
    private final Downscaler<Object> downscaler;
    private final Function<Object> function;
    @ToString.Exclude
    private final AbstractTargetTree[] arguments;

    public FunctionCall(AbstractSyntaxTree reference, Function<Object> function, List<AbstractTargetTree> arguments) {
        super(reference, arguments);
        this.function = function;
        this.upscaler = (Upscaler<Object>) reference.getType().getUpscaler();
        this.downscaler = (Downscaler<Object>) reference.getType().getDownscaler();
        this.arguments = arguments.toArray(new AbstractTargetTree[0]);
    }

    @Override
    public UpscalableIterator<Object> evaluate(Interval interval, ExecutorContext context) {
        return function.evaluate(interval, context, upscaler, downscaler, arguments);
    }
}
