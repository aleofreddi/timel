package net.vleo.timel.impl.target.tree;

/*-
 * #%L
 * TimEL core
 * %%
 * Copyright (C) 2015 - 2019 Andrea Leofreddi
 * %%
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Lesser Public License for more details.
 *
 * You should have received a copy of the GNU General Lesser Public
 * License along with this program.  If not, see
 * <http://www.gnu.org/licenses/lgpl-3.0.html>.
 * #L%
 */

import lombok.val;
import net.vleo.timel.executor.ExecutorContext;
import net.vleo.timel.impl.intermediate.tree.AbstractSyntaxTree;
import net.vleo.timel.impl.upscaler.SameUpscaler;
import net.vleo.timel.iterator.*;
import net.vleo.timel.time.Interval;
import net.vleo.timel.time.Sample;

import java.util.List;

/**
 * Compilation unit.
 *
 * @author Andrea Leofreddi
 */
public class CompilationUnit extends AbstractTargetTree {
    public CompilationUnit(AbstractSyntaxTree reference, List<AbstractTargetTree> children) {
        super(reference, children);
    }

    @Override
    public UpscalableIterator<Object> evaluate(Interval interval, ExecutorContext context) {
        int size = getChildren().size();

        for(int i = 0; i < size - 1; i++) {
            val itor = getChildren().get(i).evaluate(interval, context);
            while(itor.hasNext())
                itor.next();
        }

        return new ChopUpscalableIterator<>(
                getChildren().get(getChildren().size() - 1).evaluate(interval, context),
                interval
        );
    }
}
